...
public class CustomMultipartRequest extends Request<JSONObject> {
    ...
    private final ArrayList<File> mFilePartData;
    ...
    public CustomMultipartRequest(int method, Context mContext, String url, Response.Listener<JSONObject> listener, Response.ErrorListener errorListener, ArrayList<File> mFilePartData, Map<String, String> mStringPart, Map<String, String> mHeaderPart,String imageTitle) {
    ...
    private void buildMultipartFileEntity() {
        for (int i = 0;i<mFilePartData.size();i++) {
            try {
                File file = mFilePartData.get(i);
                String mimeType = getMimeType(mContext, file.toString());
                mEntityBuilder.addBinaryBody(imageTitle+ "["+  i + "]", file, ContentType.create(mimeType), file.getName());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
